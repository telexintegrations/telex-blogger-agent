...
namespace TelexBloggerAgent.Services
{
    public class BlogAgentService : IBlogAgentService
    {
        ...
        public async Task<string> GenerateResponse(string message, string systemMessage, string channelId, string threadId = null, string org_id = null)
        {
            ...
            if (!isAdded)
                throw new Exception("Failed to add messege to org");

            var conversations = await _messageRepository.FilterAsync(new 
            { 
                tag = CollectionType.Message 
            });
            ...
            var conversation = conversations.Where(c => c.Data.SourceId == channelId)
            ...
            var requestBody = new GeminiRequest
            {
                ...
                Contents = conversation // Send only messages for this channel
            };
            ...
            await _messageRepository.CreateAsync(AIresponse);
